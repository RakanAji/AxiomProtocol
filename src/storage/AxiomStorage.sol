...
library AxiomStorage {
    ...
    struct Storage {
        ...
        // ============ V5 Registry Enumeration (APPEND ONLY) ============

        /// @notice Global list of successfully registered V1 record IDs
        /// @dev Added at the end of the struct to preserve the existing storage layout.
        bytes32[] allRecordIds;

        // ============ V5 License Accounting (APPEND ONLY) ============

        /// @notice Number of license NFTs minted from each license template
        mapping(uint256 => uint256) licensePurchaseCount;

        /// @notice License NFT IDs owned by an account (updated on mint/transfer)
        mapping(address => uint256[]) ownedLicenseTokens;

        /// @notice Index plus one of a token in ownedLicenseTokens[owner]
        mapping(uint256 => uint256) ownedLicenseTokenIndex;

        // ============ V5 Dispute Accounting (APPEND ONLY) ============

        /// @notice Global list of dispute IDs for pagination
        bytes32[] allDisputeIds;

        /// @notice Settlement share allocated to the challenger, in basis points
        mapping(bytes32 => uint16) disputeSettlementChallengerShare;

        /// @notice Appeal deadline for an appealed dispute
        mapping(bytes32 => uint40) disputeAppealDeadlines;

        /// @notice Whether a dispute has consumed its single appeal
        mapping(bytes32 => bool) disputeAppealUsed;

        /// @notice Protocol fee snapshot taken when each dispute is initiated
        mapping(bytes32 => uint16) disputeProtocolFeeBps;

        /// @notice Native ETH reserved for unresolved dispute stakes and appeal bonds
        uint256 totalEscrowedNativeStake;

        /// @notice Minimum appeal bond snapshot taken on dispute initiation
        mapping(bytes32 => uint256) disputeMinAppealStake;

        /// @notice Evidence window snapshot taken on dispute initiation
        mapping(bytes32 => uint40) disputeEvidencePeriod;

        /// @notice Appeal window snapshot taken on dispute initiation
        mapping(bytes32 => uint40) disputeAppealPeriod;

        // ============ V5 Configuration Flags (APPEND ONLY) ============

        /// @notice Whether the dispute subsystem has been configured
        bool disputeSystemInitialized;

        /// @notice Whether the privacy verifier is explicitly allowed for production use
        bool privacyVerifierProductionApproved;
    }
     ...
     */
    function generateRecordId(bytes32 _contentHash, address _issuer) internal pure returns (bytes32) {
