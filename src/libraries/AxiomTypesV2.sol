...
library AxiomTypesV2 {
    ...
    struct StakeConfig {
        ...
        uint16 rewardBps; // Reserved; must remain zero until two-sided bonding exists
        uint16 slashBps; // Reserved; must remain zero until two-sided bonding exists
    ...
    event ContentDisputed(
        bytes32 indexed recordId, bytes32 indexed disputeId, address indexed challenger, DisputeReason reason
    );
    ...
    event VerificationLevelChanged(
        address indexed identity, VerificationLevel oldLevel, VerificationLevel newLevel, address indexed verifier
    );
     ...
     */
    event RoyaltyDistributed(bytes32 indexed recordId, address indexed recipient, uint256 amount);
     ...
     */
    event DisputeResolved(bytes32 indexed disputeId, DisputeStatus outcome, address winner);
     ...
     */
    event GDPRErasureProcessed(bytes32 indexed recordId, bytes32 indexed requestId, uint40 processedAt);
     ...
     */
    event SubscriptionActivated(address indexed enterprise, SubscriptionTier tier, uint256 credits, uint40 expiresAt);
